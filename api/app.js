...
app.use(cors({
    ...
    origin: (origin, callback) => {
        let allowedOrigins = ['http://localhost:5173', 'http://10.0.0.180:5173', 'http://10.0.0.180:3000', 'https://ideasthatfloat.test.ideasthatfloat.com', 'https://website-sql-rewrite.onrender.com/'];
        ...
        if (!userDoc) {
            ...
            if (passOk) {
                jwt.sign({
                    ...
                    res.cookie('userToken', token, { domain: hostname, sameSite: 'Lax' });
...
app.post('/sign-guest-token', async (req, res) => {
    try {
        jwt.sign({
            ...
            const hostname = url.parse(req.headers.origin).hostname;
            res.cookie('cartToken', token, { domain: hostname, sameSite: 'Lax' })
...
app.post('/sign-theme-token', async (req, res) => {
    try {
        jwt.sign({
            ...
            const hostname = url.parse(req.headers.origin).hostname;
            res.cookie('themeToken', token, { domain: hostname, sameSite: 'Lax' })
            res.status(200).json(token)
        })
