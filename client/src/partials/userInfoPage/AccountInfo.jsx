...
const AccountInfo = ({ name, setName, email, setEmail, password, setPassword, darkMode, username, needNewEmailVerification, setNeedNewEmailVerification, changedPassword, setChangedPassword }) => {
    ...
    return (
        <form className="w-[65rem] lgMobile:px-0 mdMobile:px-0 lgMobile:mt-16 mdMobile:mt-16 flex flex-col items-center px-24 lgMobile:ml-28 mdMobile:ml-28"
            onSubmit={(ev) => {
                ev.preventDefault();
                saveAccountChanges();
            }}
        >
            <h1 className="mt-[6rem] font-bold text-3xl lgMobile:text-xl mdMobile:text-xl">Account Info</h1>
            <p className="mt-6 text-2xl lgMobile:text-lg mdMobile:text-lg font-semibold">{name}</p>
            <div className="w-full lgMobile:w-1/2 mdMobile:w-1/2 h-[5rem] mt-6 flex items-center px-24 lgMobile:px-4 mdMobile:px-4 rounded-2xl border-2">
                <label htmlFor="userEmail" className="text-xl font-semibold mr-4 lgMobile:hidden mdMobile:hidden">Email:</label>
                <p className={`text-xl lgMobile:hidden mdMobile:hidden font-semibold ${darkMode ? 'text-white/50' : 'text-black/40'} ${isChangeEmail ? 'hidden' : ''}`}>{email || ''}</p>
                ...
                />
                <button className={`${isChangeEmail ? 'hidden' : ''} border-4 border-[#FF7F11] px-10 py-2 text-xl lgMobile:text-sm mdMobile:text-sm ${darkMode ? 'text-white' : 'text-black'} font-semibold rounded-full ml-auto`}
                    onClick={(ev) => {
                        ev.preventDefault();
            ...
            <div className="w-full lgMobile:w-1/2 mdMobile:w-1/2 h-[5rem] mt-6 flex lgMobile:flex-col mdMobile:flex-col lgMobile:justify-center mdMobile:justify-center items-center px-24 lgMobile:px-4 mdMobile:px-4 rounded-2xl border-2">
                <label htmlFor="userPassword" className="text-xl font-semibold mr-4 lgMobile:hidden mdMobile:hidden">Password:</label>
                <button className={`${isChangePassword ? 'hidden' : ''} border-4 border-[#FF7F11] px-6 rounded-full text-xl lgMobile:text-sm mdMobile:text-sm font-semibold ${darkMode ? 'text-white' : 'text-black'} py-2 ml-auto`}
                    onClick={(ev) => {
                        ev.preventDefault();
