        ...
        <>
            <div className={`absolute top-0 left-0 w-full h-screen bg-black/50 z-[99] flex justify-center items-center ${emailSentPopup ? '' : 'hidden'}`}>
                <div className="w-[20rem] h-[10rem] bg-[#131313] text-white border-4 rounded-xl border-[#FF7F11] flex flex-col justify-center items-center">
                    <p className="text-xl font-semibold font-hind">Email sent</p>
            ...
            <div className={`h-screen w-full flex flex-col items-center justify-center pt-32 ${darkMode ? 'bg-[#131313]' : 'bg-white'}`}>
                ...
                >
                    <p className="text-center text-3xl font-semibold font-hind">Forgot Password</p>
                    <input type="email" className="bg-transparent font-shanti border-b-2 w-4/5 text-2xl font-semibold mt-14 outline-none" placeholder="Email" required
                        onChange={(ev) => setEmail(ev.target.value)}
                    />
                    <button type="submit" className="bg-[#FF7F11] text-white font-semibold mt-auto mb-2 text-xl rounded-full w-2/3 p-2 font-hind">Send Email</button>
                </form>
            </div>
            <Footer />
        </>
